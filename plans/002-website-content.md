# #002: Website Content — Customize All Pages for Linkvec
...
- [ ] T2: Pricing page (`/pricing`) — replace demo tiers with real Linkvec tiers — **done**
- [ ] T3: Homepage (`/`) — replace demo copy with Linkvec landing content — **done**
- [ ] T4: Why Linkvec page (`/why-linkvec`) — create new page — **done**
- [ ] T5: About page (`/about`) — replace template copy with Linkvec context — **done**
...
| **T4** | `/why-linkvec`  | `src/pages/why-linkvec.astro`                | **New file**  | Live nav link; write after homepage so voice/framing is established.                                       |
...
All pages currently contain AstroWind template demo copy (references to "AstroWind", "Stellar Pricing for Every Journey", etc.). The site is live at `linkvec.com` but has no Linkvec product copy, messaging, or pricing. This plan replaces all placeholder content with production-ready Linkvec copy derived from the GTM strategy docs in `../LinkVec - GTM and Lauch/`.
...
- Positioning and value props → `../LinkVec - GTM and Lauch/Linkvec-gtm-strategy.md`
- Broker concept and security story → `../LinkVec - GTM and Lauch/Linkvec-broker-concept.md`
- Primitive names and descriptions → `../LinkVec - GTM and Lauch/Linkvec-constructs-concept.md`
...
| 2     | `Steps`        | 3 steps: (1) Install — show the curl command; (2) Expose — `Linkvec expose --port 8080 --name myservice`; (3) Connect — `Linkvec connect --service Linkvec://myservice`              |
...
**Dependency note:** The install command URL (`https://downloads.linkvec.com/linkvec-install.sh`) must exist before this page is published. Coordinate with infra (blocked by launch plan Week 1). Use a placeholder URL or GitHub releases link in the meantime.
...
Replace demo tier names and prices with the real Linkvec tier structure from GTM strategy §Pricing. Follows the existing pricing page pattern: `HeroText → Pricing → FAQs → Steps → Features3 → CallToAction`.
...
| Order | Widget         | Props summary                                                                                                                                                                           |
| ----- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | `Hero`         | headline "Connect on your terms", subtitle = one-liner, image = product screenshot/GIF (TBD), actions: "Get started free" (primary → /download) + "View docs" (link → docs.linkvec.com) |
...
| 6     | `Steps`        | 3 steps: (1) Install `curl … \| sh`, (2) `Linkvec expose --port 8080`, (3) `Linkvec hub_share` — inline code in step titles                                                             |
...
### T4: Why Linkvec page (`src/pages/why-linkvec.astro`) — NEW FILE

**Status**: todo
**Notes**:

This is the "compare + convince" page for visitors who arrive curious but not yet sold.

**Widget composition:**

| Order | Widget         | Props summary                                                                                                                                                                                                |
| ----- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1     | `Hero`         | headline "Full product. Full choice.", subtitle "Most connectivity tools make you pick one. Linkvec doesn't.", image = positioning gap diagram or product screenshot, action: "Get started free" → /download |
...
| 5     | `Features2`    | Competitor comparison — Linkvec vs ngrok / Tailscale / CF Tunnel / NetBird / WireGuard DIY. Use Features2's paired-panel capability for "them vs us" framing per competitor                                  |
...
Replace template "About us" filler. Linkvec is a solo founder project at launch; avoid corporate-speak. The audience is technical developers who will Google the founder. Follows the existing `about.astro` page structure.
...
| 2     | `Content`      | image=right, "Why Linkvec exists" — 2–3 paragraphs: CGNAT frustration, the gap between managed and DIY, the decision to build                                                  |
| 3     | `Steps`        | "What Linkvec believes" — 3 principles: (1) Choice is a feature, not a trade-off. (2) Your data plane should be yours. (3) Security should be structural, not just configured. |
| 4     | `Features3`    | 3 columns: GitHub (link to repo), Discord (community), Docs (docs.linkvec.com) — the open presence                                                                             |
...
| 2     | `Features3` | 3 contact channels: Email (`hi@linkvec.com`), Discord (community link), GitHub Discussions (primary support for Free tier) |
...
- External links: `https://docs.linkvec.com`, `https://status.linkvec.com`, `https://github.com/linkvec` — verify live before launch.
...
1. **Download page hosting** — `https://downloads.linkvec.com/linkvec-install.sh` needs to exist before the download page goes live. Is this blocked on infra (Week 1 in launch plan)?
